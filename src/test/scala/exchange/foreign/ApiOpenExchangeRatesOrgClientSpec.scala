...
import exchange.foreign.client.{FixerIoClient, OpenExchangeRatesOrgClient}
...
class ApiOpenExchangeRatesOrgClientSpec extends WordSpec
  ...
  val client = OpenExchangeRatesOrgClient(config.getConfig("app.client.openexchangeratesorg"))(system, materializer)
  implicit val apiExchangeManager = system.actorOf(ExchangeManager.props(client))
  lazy val routes = apiRoutes()

  "ApiRoutes for openexchangerates.org client" should {
    "return rates (GET /)" in {
...
//      implicit val timeout = RouteTestTimeout(2.seconds)
      request ~> routes ~> check {
        ...
        //        val response = entityAs[ApiRoute.SymbolsResponse](3.seconds)
